...
static void
class_m5_version(mrb_vm *vm, mrb_value *v, int argc)
{  // a little small challenge
    SET_RETURN(mrbc_string_new_cstr(vm, VER_N_STR(M5UNIFIED_VERSION_MAJOR) "." VER_N_STR(M5UNIFIED_VERSION_MINOR) "." VER_N_STR(M5UNIFIED_VERSION_PATCH))); 

//    char buf[32];
//    snprintf(buf, sizeof(buf), "%d.%d.%d", M5UNIFIED_VERSION_MAJOR, M5UNIFIED_VERSION_MINOR, M5UNIFIED_VERSION_PATCH);
